...
- [x] **2.0 Confirm baseline green.** Run `npm run typecheck` and `npm test`;
...
- [x] **2.1a Verify the existing division** in `src/engine/cascade/groups.ts`
...
- [x] **2.1b Add the deck-4 p.65 golden** `A+B+D-A-B-C+C-D+`: assert the
  ...
  (REQ-CAS-MERGE-DECK)
- [x] **2.1c Add the user example golden** `A+B+A-B-` (G1) at the logical level
  (groups `[A+ B+] [A- B-]`, `Nm=1`). (REQ-CAS-DIV-RULE)

## 2.2 CascadePlan / model completeness

- [x] **2.2a Reconcile the model** in `src/engine/cascade/model.ts` /
  ...
  (REQ-CAS-* model)
- [x] **2.2b Add a derived `transitions` view** (`CascadeTransition` with
...
- [x] **2.2c Populate `CascadeGroup.stepIndices`** in
...
- [x] **2.3a Cascade-specific checks** (extend `src/validator/` or add a cascade
...
- [x] **2.3b Confirm the generic validator** still accepts every golden circuit
...
- [x] **2.4a Verify `toCascadeCircuit`** emits the full pneumatic structure
...
- [x] **2.4b Golden circuit assertions** for G1 and G5 at the circuit level
...
- [x] **2.5 Cascade layout** in `src/layout/`: place group pressure-line buses
...
- [x] **2.6 Cascade pneumatic SVG** in `src/renderer/pneumatic`: draw cylinders,
...
- [x] **2.7a Keep the group listing** golden (`Grupo I / … / Nm = k`) in
  `tests/golden/cascade.golden.test.ts` passing. (REQ-CAS-EXPLAIN)
- [x] **2.7b Add the chronological narrative** derived from the logical model +
...
- [x] **2.8a Confirm `generate()`** routes `method:'cascade'` to
...
- [x] **2.8b Keep `Cascata` selectable in the UI** and routed only through the
  pipeline facade (best-effort; not runnable in the sandbox). Documentation-level
  confirmation. (REQ-CAS-UI)

## 2.9 Property tests

- [x] **2.9 Add the six property tests** under `tests/properties/` using the
...
- [x] **2.10 Full green.** `npm run typecheck` clean AND `npm test` green
